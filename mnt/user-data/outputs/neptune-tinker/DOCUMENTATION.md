...
The sandbox is managed via a `docker-compose.yml` in `scripts/`, exposed through npm scripts. All configuration is via environment variables with sensible defaults:
...
Set these inline, in a `.env` file next to the compose file, or in your shell profile. See `scripts/.env.example` for a template.
...
The `--wait` flag on `docker compose up` blocks until the healthcheck passes, so by the time the npm script returns, the Gremlin endpoint is ready. The compose file mounts a static `gremlin-server.yaml` (port 8182 inside the container, mapped to `NEPTUNE_TINKER_PORT` on the host).

Consumers who need more control can use the compose file directly, extend it, or bring their own Gremlin Server — the TypeScript library only needs a WebSocket endpoint.
...
# Verify (--wait blocks until healthy, but just in case)
